...
-- 매니저
DROP TABLE IF EXISTS lms_mamber RESTRICT;
...
-- 교육센터
DROP TABLE IF EXISTS lms edu center RESTRICT;
...
CREATE TABLE lms_lecture (
  ...
  context    LONGTEXT     NOT NULL COMMENT '내용', -- 내용
  ...
  member_id  DATETIME     NULL     COMMENT '매니저번호', -- 매니저번호
...
CREATE TABLE lms_student (
  student_id DATETIME NOT NULL COMMENT '학생번호', -- 학생번호
  working    INTEGER  NOT NULL COMMENT '재직여부', -- 재직여부
  birthday   CHAR(6)  NOT NULL COMMENT '생년원일' -- 생년원일
)
COMMENT '학생';
...
CREATE TABLE lms_teacher (
  teacher_id DATETIME NOT NULL COMMENT '강사번호', -- 강사번호
...
-- 매니저
CREATE TABLE lms_mamber (
  member_id      DATETIME    NOT NULL COMMENT '매니저번호', -- 매니저번호
  deptartment_id INTEGER     NOT NULL COMMENT '부서번호', -- 부서번호
...
-- 매니저
ALTER TABLE lms_mamber
  ADD CONSTRAINT PK_lms_mamber -- 매니저 기본키
  PRIMARY KEY (
  member_id -- 매니저번호
  );
...
CREATE TABLE lms_class (
  ...
  edu_center_id INTEGER     NOT NULL COMMENT '센터교육번호', -- 센터교육번호
  name          VARCHAR(60) NOT NULL COMMENT '강의실명', -- 강의실명
  capacithy     INTEGER     NOT NULL COMMENT '수용인원' -- 수용인원
)
COMMENT '강의실';
...
CREATE TABLE lms_member (
  member_id      DATETIME     NOT NULL COMMENT '회원번호', -- 회원번호
  name           VARCHAR(60)  NOT NULL COMMENT '이름', -- 이름
  tel            VARCHAR(30)  NOT NULL COMMENT '전화', -- 전화
  email          VARCHAR(40)  NOT NULL COMMENT '이메일', -- 이메일
  arddr_id       INTEGER      NOT NULL COMMENT '기본주소번호', -- 기본주소번호
  detail_daaress VARCHAR(255) NULL     COMMENT '상세주소', -- 상세주소
  drgree_id      INTEGER      NOT NULL COMMENT '학력번호', -- 학력번호
...
ALTER TABLE lms_member
  MODIFY COLUMN member_id DATETIME NOT NULL AUTO_INCREMENT COMMENT '회원번호';

-- 교육센터
CREATE TABLE lms edu center (
  edu_center_id  INTEGER      NOT NULL COMMENT '센터교육번호', -- 센터교육번호
  name           VARCHAR(60)  NOT NULL COMMENT '센터명', -- 센터명
  arddr_id       INTEGER      NOT NULL COMMENT '기본주소번호', -- 기본주소번호
  detali_address VARCHAR(255) NOT NULL COMMENT '센터상세주소', -- 센터상세주소
...
-- 교육센터
ALTER TABLE lms edu center
  ADD CONSTRAINT PK_lms edu center -- 교육센터 기본키
  PRIMARY KEY (
  edu_center_id -- 센터교육번호
  );

-- 교육센터 유니크 인덱스
CREATE UNIQUE INDEX UIX_lms edu center
  ON lms edu center ( -- 교육센터
    name ASC -- 센터명
  );

ALTER TABLE lms edu center
  MODIFY COLUMN edu_center_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '센터교육번호';
...
ALTER TABLE lms_department
  MODIFY COLUMN deptartment_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '부서번호';
...
CREATE TABLE lms_class_photo (
  class_poto_id   INTEGER      NOT NULL COMMENT '강의실 사진번호', -- 강의실 사진번호
  photo_path      VARCHAR(255) NOT NULL COMMENT '사진', -- 사진
  origin_filename VARCHAR(60)  NOT NULL COMMENT '얼레사진파일명', -- 얼레사진파일명
  mimetype        VARCHAR(60)  NOT NULL COMMENT 'MIME YTPE', -- MIME YTPE
  class_id        INTEGER      NOT NULL COMMENT '강의실번호' -- 강의실번호
)
COMMENT '강의실 사진';
...
ALTER TABLE lms_class_photo
  ...
  PRIMARY KEY (
  class_poto_id -- 강의실 사진번호
  );

ALTER TABLE lms_class_photo
  MODIFY COLUMN class_poto_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '강의실 사진번호';

-- 기본주소
CREATE TABLE lms_addr (
  arddr_id      INTEGER      NOT NULL COMMENT '기본주소번호', -- 기본주소번호
  pastno        VARCHAR(10)  NOT NULL COMMENT '운편번호', -- 운편번호
...
ALTER TABLE lms_addr
  MODIFY COLUMN arddr_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '기본주소번호';

-- 수강신청
CREATE TABLE lms_application (
  lecture_id           INTEGER     NOT NULL COMMENT '강의번호', -- 강의번호
  student_id           DATETIME    NOT NULL COMMENT '학생번호', -- 학생번호
  created_id           DATETIME    NULL     DEFAULT now() COMMENT '신청일', -- 신청일
...
-- 강의
ALTER TABLE lms_lecture
  ADD CONSTRAINT FK_lms_mamber_TO_lms_lecture -- 매니저 -> 강의
  FOREIGN KEY (
  member_id -- 매니저번호
  )
  REFERENCES lms_mamber ( -- 매니저
  member_id -- 매니저번호
  );
...
-- 매니저
ALTER TABLE lms_mamber
  ADD CONSTRAINT FK_lms_member_TO_lms_mamber -- 회원 -> 매니저
  FOREIGN KEY (
  member_id -- 매니저번호
  )
  REFERENCES lms_member ( -- 회원
...
-- 매니저
ALTER TABLE lms_mamber
  ADD CONSTRAINT FK_lms_department_TO_lms_mamber -- 부서 -> 매니저
  FOREIGN KEY (
  deptartment_id -- 부서번호
  )
...
-- 강의실
ALTER TABLE lms_class
  ADD CONSTRAINT FK_lms edu center_TO_lms_class -- 교육센터 -> 강의실
  FOREIGN KEY (
  edu_center_id -- 센터교육번호
  )
  REFERENCES lms edu center ( -- 교육센터
  edu_center_id -- 센터교육번호
  );
...
-- 교육센터
ALTER TABLE lms edu center
  ADD CONSTRAINT FK_lms_addr_TO_lms edu center -- 기본주소 -> 교육센터
  FOREIGN KEY (
  arddr_id -- 기본주소번호
  )
  REFERENCES lms_addr ( -- 기본주소
